...
class PIMMDecoder(nn.Module):
    def __init__(
        ...
        for d in range(self.sam_features_length):
            in_channels = endoder_transformer_dim[d]
    ...
    def forward(
        ...
        for d in range(len(self.conv_blocks_context)):#这个是三层的,d=0~2
            input=image_embeddings[-(d + 1)]
            embed = self.conv_blocks_context[d](input)
            if d == 0:
...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        endoder_transformer_dim: int = [320,320,160],  #输入maskdecoder的三层维度（最后到最前）
        upsample_transformer_dim: int = 256, 
        sam_features_length: int = 3,
        do_deep_supervision: bool = False,
    ) -> None:
...
class MaskDecoder_sammed(nn.Module):
    def __init__(
        self,
        *,
        transformer_dim: int,
